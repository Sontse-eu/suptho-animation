...
const useSetup = (dimensions, vals) => {
  const numCircles = vals.length;
  const circles = useMemo(() => {
    ...
    for (let i = 0; i < numCircles; i++) {
      ...
      circles.push({ x: new Value(x), y: new Value(y), dat: vals[i] });
    }
    return circles;
  }, [dimensions]);
...
export const useGravityAnimation = (dimensions, vals) => {
  const circles = useSetup(dimensions, vals);
  useDraw(circles, vals);
